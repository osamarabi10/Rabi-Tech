...
function InvitationForm() {
  ...
  if (accepted) return <div className="space-y-5 text-center"><CheckCircle2 className="mx-auto size-10 text-success" /><div><h1 className="text-xl font-semibold">{t('You joined the organization')}</h1><p className="mt-2 text-small text-muted-foreground">{t('Your account is ready. Sign in with your email and password.')}</p></div><Button asChild className="w-full"><Link href="/login">{t('Go to sign in')}</Link></Button></div>;

  return <div className="space-y-5">
    <div><h1 className="text-xl font-semibold">{t('Join organization')}</h1><p className="mt-2 text-small text-muted-foreground">{invitation.invitedByName} {t('invited you to join')} <strong className="text-foreground">{invitation.workspaceName}</strong>.</p></div>
    ...
    <div className="space-y-1.5"><Label htmlFor="accepted-password">{t(invitation.requiresExistingPassword ? 'Current account password' : 'Create password')}</Label><Input id="accepted-password" type="password" dir="ltr" value={password} onChange={(event) => setPassword(event.target.value)} /><p className="text-micro text-muted-foreground">{t(invitation.requiresExistingPassword ? 'Use your existing RabiTech password to add this organization.' : 'Use at least 10 characters.')}</p></div>
