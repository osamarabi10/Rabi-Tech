...
type EditForm = {
  ...
  /*
    Four restrictions the backend has enforced since M8.1 with no control
    anywhere in the console.

    They were reachable only by calling the API by hand, which means no admin
    could actually apply them — the enforcement was real and the feature was
    not. Found by auditing the restriction table against the UI that writes it.
  */
...
export function OrganizationUsers() {
  ...
  const load = useCallback(async () => {
    ...
    setFailed(false);
    try {
      const [roster, teamRows, seatUsage] = await Promise.all([fetchOrganizationUsers(), fetchTeams(), fetchSeatUsage()]);
      ...
      setFailed(true);
      if (error?.response?.status === 403) toast.error(t('You do not have access to organization users'));
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  const saveUser = async () => {
    ...
    try {
      await updateSystemUser(selected.id, {
        ...
        restrictDataExport: form.restrictDataExport,
        restrictContactDeletion: form.restrictContactDeletion,
        restrictWorkspaceSettings: form.restrictWorkspaceSettings,
        restrictIntegrations: form.restrictIntegrations,
      });
  ...
  if (loading) return <LayoutSkeleton label={t('Loading organization users')} className="m-4" />;
  if (failed) return <ErrorState title={t('Could not load organization users')} retryLabel={t('Try again')} onRetry={load} className="m-4" />;
