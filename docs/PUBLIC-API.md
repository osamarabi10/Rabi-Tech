...
| `conversations:read` | Read conversations *(P1c)* |
| `conversations:write` | Assign, close, reopen *(P1c)* |
| `messages:read` | Read messages *(P1c)* |
| `messages:send` | Send messages *(P1c)* |
| `tags:read` | Read tags |
...
58 checks over HTTP against the running server, because everything that makes
this surface safe lives in the middleware chain rather than in the handlers.
Mutation-proved: leaking `organizationId` from the serializer and unscoping the
id lookup both take it red.

---

## Roadmap

P1c adds conversations and messaging; P1d adds outbound webhooks with
