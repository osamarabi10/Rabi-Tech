...
| File | The rule |
|---|---|
| `inbox/inbox-selector.tsx` | Pane 1: scope, and the only always-visible gateway state on a wide screen. Counts come from the conversations actually loaded — exact today because the list endpoint has no pagination, and documented in the file as the thing that must move server-side when it gains any. Also owns saved views: the group, the row menu, and the save control. **Every row derives count, active and onSelect from one scope literal** via `row()`; writing any of the three by hand is how a row comes to count one thing and open another. |
| `inbox/inbox-scope-menu.tsx` | The same scopes and the same counting for screens below `lg`, where pane 1 is hidden, plus `GatewayNotice`. Not a duplicate: without it those scopes exist on desktop only. **Group order must match the pane** — they disagreed once (views before teams here, after teams there) and it read as two different products. |
| `lib/inbox-view-match.ts` | Evaluates a saved view against a loaded `Conv`. **Must never be more permissive than the server validator** in `apps/backend/src/lib/inbox-view-filter.ts`: a key rejected there can never arrive here, and a key accepted there must be honoured here or the view silently ignores part of what its author asked for. |
| `lib/inbox-view-capture.ts` | Turns the live inbox filter — scope, status pill, label chip — into a storable filter, and reports what it *cannot* store. Mentions and search have no expressible form; both are named to the user rather than dropped. |
| `inbox/save-view-dialog.tsx` | Creating and renaming a view. One dialog for both, because they ask the same two questions. Shows the capture summary so nobody saves a view that means something other than what is on screen. |
